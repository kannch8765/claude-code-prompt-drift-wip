...
const scriptPath = "scripts/apply-task-003-review-fixes.mjs";

function replaceOnce(content, before, after, path) {
  const first = content.indexOf(before);
  assert.notEqual(first, -1, `${path}: expected source block was not found`);
  assert.equal(
    content.indexOf(before, first + before.length),
    -1,
    `${path}: expected source block was not unique`,
  );
...
const oldRedactor = `function redactAbsolutePaths(value) {
  return value
    .replace(
      /(^|[\\s("'\\`])(?:[A-Za-z]:[\\\\/]|\\\\\\\\[^\\\\/\\s]+[\\\\/]|\\/(?!\\/))[^\\s<>()\\[\\]{}]*/gu,
...
renderer = replaceOnce(renderer, oldRedactor, newRedactor, rendererPath);
...
test("renderer redacts key-prefixed absolute and UNC paths without damaging HTTPS", () => {
  const message = [
    ...
    String.raw\\`artifact=C:\\private\\prompt.md\\`,
    String.raw\\`artifact=\\\\server\\share\\secret\\`,
  ...
  for (const leaked of [
    ...
    String.raw\\`C:\\private\\prompt.md\\`,
